...
gridOptions.api.applyTransaction({remove:[{id_risorse_practice: arg.id_t }]});
...
{    console.log(ListaValori)
  console.log(riga)
    ...
    var id_risorse_practice = ListaValori[riga].id_risorse_practice
    ...
    console.log(id_risorse_practice+ nome + cognome + email + id_risorsa + " " + id_practice + practice + data_validita)
    var jsonVar = {"query" : ("delete  from rilatt.risorse_practice  where true and  id_risorsa = '" +id_risorsa + "' and id_practice = '"+
     id_practice+ "' and  dtvalid_risorsa_practice = '"+ data_validita + "'").replace("= 'null'"," is NULL"),richiesta : contatoreDelete++ ,id_t : id_risorse_practice , riga : riga}
...
var stampaCoordCella = function(){
 ...
 function sendInsert(risorsa , practice , data ) {
     ...
     jsonVar = {"query" : "insert into rilatt.risorse_practice (id_risorsa, id_practice , dtvalid_risorsa_practice) values ('"+risorsa+"','"+practice+"','"+data+"' ) "}
    ...
    sendInsertSelect(ris[0],ris[1],ris[2], lvl[1] , data)
     ...
     function sendInsertSelect(nome , cognome , email , practice , data ) {
      ...
      if (practice != "" && practice != undefined )
       ...
       {
              condition5 = " and dtvalid_risorsa_practice = '" + data+"' "
       }
       ...
       if (email != "" && email != undefined )
       {
         ...
         jsonVar = {"query" : "Select id_risorse_practice,descrizione as practice, r.id_risorsa, l.id_practice,to_char( dtvalid_risorsa_practice, 'YYYY-MM-DD') as data_validita,nome,cognome,email from rilatt.risorse_practice rl " +
